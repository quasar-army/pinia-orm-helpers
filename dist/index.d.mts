import { Model, Type, Attribute, Relation } from 'pinia-orm';
...
declare function getClassRelationships<ModelType extends typeof Model>(PiniaOrmClass: ModelType): Record<{ [K in keyof DeclassifyPiniaOrmModel<InstanceType<ModelType>>]: DeclassifyPiniaOrmModel<InstanceType<ModelType>>[K] extends Model | Model[] | null ? K : never; }[Exclude<keyof RemoveIndex<InstanceType<ModelType>>, "_meta" | "$self" | "$entity" | "$config" | "$baseEntity" | "$typeKey" | "$types" | "$piniaOptions" | "$primaryKey" | "$fields" | "$hidden" | "$visible" | "$newInstance" | "$casts" | "$fill" | "$getKeyName" | "$getKey" | "$hasCompositeKey" | "$getIndexId" | "$getLocalKey" | "$getRelation" | "$setRelation" | "$getMutators" | "$getCasts" | "$getOriginal" | "$refresh" | "$isDirty" | "$getAttributes" | "$toJson">], Relation & {
    kind: PiniaOrmDecoratorKind;
    isRelationship: boolean;
    key: string;
}>;
...
export { type DeclassifyPiniaOrmModel, type DeclassifyPiniaOrmModels, type FilterPiniaOrmModelToFieldTypes, type FilterPiniaOrmModelToRelationshipTypes, type PiniaOrmDecoratorKind, type PiniaOrmForm, type RemoveIndex, getClassAttributes, getClassFields, getClassRelationships };
